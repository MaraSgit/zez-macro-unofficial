...
if !FileExist("icon.ico"){
    MsgBox Failure reading the icon file. This is either because the files were never extracted, the file is missing, or ahk doesnt have permission to read the file.
    ExitApp
}
if !FileExist("config.ini"){
    MsgBox Failure reading the config file. This is either because the files were never extracted, the file is missing, or ahk doesnt have permission to read the file.
    ExitApp
}
if !FileExist("image_assets"){
    MsgBox Failure reading the image assets. This is either because the files were never extracted, the files are missing, or ahk doesnt have permission to read the files.
    ExitApp
}
#NoEnv
...
OnExit, GetOut
Gui, Add, Text, x440 y265 w190 h20 +BackgroundTrans, Vers. 0.9.04
...
Gui, Add, Text, x105 y145 +Center,Gabriel N.
Gui, Add, Text, x105 y160 +Center,Max E.
Gui, Add, Text, x105 y175 +Center,gsmithsax#0539
Gui, Add, Text, x105 y190 +Center,Hefty#4694
Gui, Add, Text, x105 y205 +Center,Alexa S.
Gui, Add, Text, x105 y220 +Center,Kapin#7262
Gui, Add, Text, x255 y85 +Center,Dario W.
Gui, Add, Text, x255 y100 +Center,ploppy#9769
Gui, Add, Text, x255 y115 +Center,ssilz#8403
...
boosting(hive:=false){
    ...
    for i, hot in hotbars{
        ...
        if (hotTimer && hive && atHive%indexer% = "yes"){
...
wrappedSearch(fileName,v,aim := "full"){
    ...
    ImageSearch, FoundX, FoundY, xi, yi, ww, wh, *%v% image_assets\%resolution%\%graphics%\%fileName%
    if (ErrorLevel = 2){
...
killCounter_(bug,amount){
    ...
    for key, value in questList.riley[RQA.title] {
        if (InStr(key,"killcount") && value[1] = bug){
            IniRead, killcount, config.ini, rquests, %key%
            killcount := killcount + amount
            IniWrite, %killcount%, config.ini, rquests, %key%
            RQA := rqaCreate()
            rqaT := questFormat3_(RQA)
            GuiControl,,rileyLog,Current Riley Quest: `n%rqaT%
        }
